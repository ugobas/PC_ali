...
double rmsd_mclachlan_f(float **rot_out, float *shift_out,
			float *x1, float *x2, float *wt1, int n)
{
  int i=0,j=0;
  double rot[3][3], cent1[3], cent2[3];

  // compute centre of masses for the reference and the mobile protein
  center_of_mass(cent1,x1,n,wt1);
  center_of_mass(cent2,x2,n,wt1);

  // Set center of mass at 0
  float tmpatom1[3*n], *x=tmpatom1, *y=x1;
  for(i=0; i<n; i++){
    for(j=0; j<3; j++){*x=*y-cent1[j]; x++; y++;}
  }
  x=x2;
  for(i=0; i<n; i++){
    for(j=0; j<3; j++){*x-=cent2[j]; x++;}
  }
  
  // compute the superposition
  matfit(tmpatom1,x2,rot,n,wt1,1);

  for (i=0;i<n;i++){
    Apply3Matrix2dot(x2+3*i,rot);
  }
  x=x2;
  for(i=0; i<n; i++){
    for(j=0; j<3; j++){*x+=cent1[j]; x++;}
  }

  // computes weighted rmsd
  double rmsd=rmsd_w(x1,x2,n,wt1);
  ...
  if(0){
    ...
    for (i=0;i<3;i++){
      printf("\t");
      for (j=0;j<3;j++)printf("%.4f ",rot[i][j]);
      printf("\n");
    }
    printf("Translation vector (between Centres of masses)\n");
    printf("\t%.4f %.4f %.4f\n",
	   cent1[0]-cent2[0],cent1[1]-cent2[1],cent1[2]-cent2[2]);
    printf("\t mass-weighted RMS: %.4f\n", rmsd);
  }

  // Output matrix
  for (i=0;i<3;i++){
    if(rot_out){for (j=0;j<3;j++)rot_out[i][j]=rot[i][j];}
    if(shift_out){
      shift_out[i]=cent1[i];
      for (j=0;j<3;j++){shift_out[i]-=rot[i][j]*cent2[j];}
    }
  }
             ...
             short     column)     /* Column-wise output   */
{
   ...
   int  i,j;
   double umat[3][3];
   for(i=0;i<3;i++){for(j=0;j<3;j++) umat[i][j] = 0.0;}
   ...
   for(int k=0; k<n; k++){
     if(wt1){
       ...
       for(i=0;i<3;i++){
	 for(j=0;j<3;j++){
	   umat[i][j] += w*x1[m+i] * x2[m+j];
	 }
       }
     }else{
       for(i=0;i<3;i++){
	 for(j=0;j<3;j++){
	   umat[i][j] += x1[m+i] * x2[m+j];
	 }
       }
     }
     m+=3;
